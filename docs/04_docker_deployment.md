...
`docker compose --profile dev up -d jupyter` 后访问 http://localhost:8888，
直接输入密码登录（密码与本地各服务一致，不写入文档；哈希已配置在
docker-compose.yml 的 jupyter command 中）。

> 排坑：compose 的 command 里 argon2 哈希的 `$` 必须写成 `$$`，
> 否则会被 Compose 当作变量替换吞掉，登录时报 InvalidHashError 500。
